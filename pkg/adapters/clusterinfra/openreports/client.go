...
func (p *reportClient) GenerateReport(ctx context.Context, report *v1alpha1.ClusterInfraAssessmentReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		polr, err := p.k8sClient.ClusterReports().Get(ctx, clusterinfra.GenerateReportName(report.Name), v1.GetOptions{})
		...
		if err != nil {
			return fmt.Errorf("failed to create ClusterReport: %s", err)
		}
...
func (p *reportClient) DeleteReport(ctx context.Context, report *v1alpha1.ClusterInfraAssessmentReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		err := p.k8sClient.ClusterReports().Delete(ctx, clusterinfra.GenerateReportName(report.Name), v1.DeleteOptions{})
