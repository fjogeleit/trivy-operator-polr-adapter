...
func (p *reportClient) GenerateReport(ctx context.Context, report *v1alpha1.ExposedSecretReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		polr, err := p.k8sClient.Reports(report.Namespace).Get(ctx, exposedsecret.GenerateReportName(report), v1.GetOptions{})
		...
		if err != nil {
			return fmt.Errorf("failed to create report in namespace %s: %s", report.Namespace, err)
		}
...
func (p *reportClient) DeleteReport(ctx context.Context, report *v1alpha1.ExposedSecretReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		err := p.k8sClient.Reports(report.Namespace).Delete(ctx, exposedsecret.GenerateReportName(report), v1.DeleteOptions{})
