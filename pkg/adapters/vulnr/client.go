...
func (e *Client) watch(ctx context.Context) {
	...
	informer := factory.Aquasecurity().V1alpha1().VulnerabilityReports().Informer()

	informer.SetWatchErrorHandler(func(_ *cache.Reflector, _ error) {
		cancel()

		log.Println("[WARNING] Watch VulnerabilityReports failed - restarting")
	})

	go e.handleCRDRegistration(ctx, informer, "VulnerabilityReport")

	informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			if report, ok := obj.(*v1alpha1.VulnerabilityReport); ok {
				err := e.polrClient.GenerateReport(ctx, report)
				if err != nil {
					log.Printf("[ERROR] VulnerabilityReports: failed to process report %s; %s", report.Name, err)
				}
			}
		},
		...
		DeleteFunc: func(obj interface{}) {
			if report, ok := obj.(*v1alpha1.VulnerabilityReport); ok {
				...
				if err != nil {
					log.Printf("[ERROR] VulnerabilityReports: failed to delete report %s; %s", report.Name, err)
				}
			}
		},
	})
