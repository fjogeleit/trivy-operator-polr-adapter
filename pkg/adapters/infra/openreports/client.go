...
func (p *reportClient) GenerateReport(ctx context.Context, report *v1alpha1.InfraAssessmentReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		polr, err := p.k8sClient.Reports(report.Namespace).Get(ctx, infra.GenerateReportName(report.Name), v1.GetOptions{})
...
func (p *reportClient) DeleteReport(ctx context.Context, report *v1alpha1.InfraAssessmentReport) error {
	return retry.RetryOnConflict(retry.DefaultRetry, func() error {
		err := p.k8sClient.Reports(report.Namespace).Delete(ctx, infra.GenerateReportName(report.Name), v1.DeleteOptions{})
