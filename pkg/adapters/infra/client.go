...
func (e *Client) StartWatching(ctx context.Context) error {
	return e.client.Watch(source.Kind(e.manager.GetCache(), &v1alpha1.InfraAssessmentReport{}), &handler.EnqueueRequestForObject{}, predicate.Funcs{
		CreateFunc: func(event event.CreateEvent) bool {
			if report, ok := event.Object.(*v1alpha1.InfraAssessmentReport); ok {
				err := e.polrClient.GenerateReport(ctx, report)
				if err != nil {
					log.Printf("[ERROR] InfraAssessmentReport: Failed to process report %s; %s", report.Name, err)
				}
			}
		...
		UpdateFunc: func(event event.UpdateEvent) bool {
			if report, ok := event.ObjectNew.(*v1alpha1.InfraAssessmentReport); ok {
				err := e.polrClient.GenerateReport(ctx, report)
				if err != nil {
					log.Printf("[ERROR] InfraAssessmentReport: Failed to process report %s; %s", report.Name, err)
				}
		...
		DeleteFunc: func(event event.DeleteEvent) bool {
			if report, ok := event.Object.(*v1alpha1.InfraAssessmentReport); ok {
				...
				if err != nil {
					log.Printf("[ERROR] InfraAssessmentReport: Failed to delete report %s; %s", report.Name, err)
				}
			}
