...
func (e *Client) watch(ctx context.Context) {
	...
	informer := factory.Aquasecurity().V1alpha1().CISKubeBenchReports().Informer()

	informer.SetWatchErrorHandler(func(_ *cache.Reflector, _ error) {
		cancel()

		log.Println("[WARNING] Watch CISKubeBenchReport failed - restarting")
	})

	go e.handleCRDRegistration(ctx, informer, "CISKubeBenchReport")

	informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			if report, ok := obj.(*v1alpha1.CISKubeBenchReport); ok {
